...
int main(int argc, char** argv)
{
  ...
  visual_tools.publishText(text_pose, node_name, rvt::WHITE, rvt::XLARGE);
  ...
  // end-effector.
  //
  // NOTE: This fails for the time being. I think that it will be easier to get
  ...
  ROS_INFO_NAMED(node_name, "Visualizing plan 1 (pose goal) %s", success ? "" : "FAILED");

  // Visualizing plans
  // ^^^^^^^^^^^^^^^^^
  // We can also visualize the plan as a line with markers in RViz.
  ROS_INFO_NAMED(node_name, "Visualizing plan 1 as trajectory line");
  ...
  // not use that function when not working with a physical robot
  // since it is a blocking function and requires a controller to
  // be active and report success on execution of a trajectory.
  ...
  joint_group_positions[1] = 0.54;  // radians
  ...
  ROS_INFO_NAMED(node_name, "Visualizing plan 4 (Cartesian path) (%.2f%% acheived)", fraction * 100.0);
  ...
  ROS_INFO_NAMED(node_name, "Add an object into the world");
  ...
  success = (move_group.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED(node_name, "Visualizing plan 5 (pose goal move around cuboid) %s", success ? "" : "FAILED");
  ...
  ROS_INFO_NAMED(node_name, "Attach the object to the robot");
  ...
  ROS_INFO_NAMED(node_name, "Detach the object from the robot");
  ...
  ROS_INFO_NAMED(node_name, "Remove the object from the world");
